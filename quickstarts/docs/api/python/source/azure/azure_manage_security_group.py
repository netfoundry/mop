...
if __name__ == '__main__':
    ...
    parser.add_argument("--action", choices=['update'],
                        help="Action you want to do on a given security group",
                        required=True)
    parser.add_argument("--sg_name",
                        help="Security Group Name, more specificaly the prefix, e.g. \
                            ${PREFIX}-securityGroup.",
                        required=True)
    parser.add_argument("--rules", nargs='+',
                        help='List of rules (port, protocol) to add to a given security \
                            group , e.g. [ "tcp,22" "tcp,443" ]',
                        required=True)
